...
public class ClientThread extends Thread {
    ...
    public ClientThread(String IP, int PORT, ObservableSubject<ClientMessage> observableMessage, PublishSubject<ClientResult> observableMessageOut, SSLContext sslContext) {
        this.IP = IP;
        this.port = PORT;
        ...
        if (message.isAwaitResponse()) {
            ...
            while ((line = bufferedReader.readLine()) != null) { // reading response after sending our message, blocking until null
                ...
                if (line.trim().startsWith(HTTP_200)) { // TODO what *should* we do if it's not 200?
                    ...
                    line = line.replace(HTTP_200, ""); // don't need to forward response code for processing, only remainder (whatever it is)
